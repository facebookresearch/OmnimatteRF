...
import cv2
from third_party.omnimatte.third_party.data.base_dataset import BaseDataset
from third_party.omnimatte.third_party.data.image_folder import make_dataset
...
class OmnimatteDataset(BaseDataset):
    ...
    def get_background_flow(self, index, w, h):
        ...
        return get_background_flow(
                self.homographies[index], self.homographies[index+1], self.h_scale_x, self.h_scale_y, w, h)

    def get_background_uv(self, index, w, h):
        """Return background layer UVs at 'index' (output range [0, 1])."""
        return get_background_uv(self.homographies[index], self.h_scale_x, self.h_scale_y, self.h_bounds_x, self.h_bounds_y, w, h)
...
def get_background_flow(H_0, H_1, sx, sy, w, h):
    ramp_u = torch.linspace(0, sx, steps=w).unsqueeze(0).repeat(h, 1)
    ramp_v = torch.linspace(0, sy, steps=h).unsqueeze(-1).repeat(1, w)
...
def get_background_uv(H, sx, sy, bx, by, w, h):
    """Return background layer UVs at 'index' (output range [0, 1])."""
