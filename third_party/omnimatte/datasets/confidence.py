...
if __name__ == "__main__":
    ...
    arguments.add_argument('--rgb', default=None)
    arguments.add_argument('--scale', default=1, type=float)
    ...
    rgb_root = opt.rgb
    if rgb_root is None:
        rgb_root = os.path.join(opt.dataroot, 'rgb')
    rgb_paths = sorted(glob.glob(os.path.join(rgb_root, '*')))
    print(f'generating {len(forward_flo)} confidence maps...')
    outdir = os.path.join(opt.dataroot, 'confidence')
    os.makedirs(outdir, exist_ok=True)
    for i in tqdm(range(len(forward_flo))):
