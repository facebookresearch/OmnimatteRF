...
def main(argv=None):
    ...
    arguments.add_argument(
        "--homography_path", type=str, help="path to file containing homographies"
    )
    arguments.add_argument("--width", type=int, help="video width")
    arguments.add_argument("--height", type=int, help="video height")
    opt = arguments.parse_args(argv)

    if opt.homography_path.endswith(".npy"):
        homographies = np.load(opt.homography_path)
        lines = [
            " ".join([str(v) for v in H.reshape([-1])]) + "\n" for H in homographies
        ]
    else:
    ...
    out_path = f"{opt.homography_path[:-4]}.txt"
