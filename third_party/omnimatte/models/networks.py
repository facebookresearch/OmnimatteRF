...
from third_party.omnimatte.third_party.models.networks import init_net
from third_party.omnimatte.third_party.models.networks_lnr import ConvBlock


###############################################################################
# Helper Functions
###############################################################################
def define_omnimatte(nf=64, in_c=16, norm="batch", gpu_ids=[]):
...
class Omnimatte(nn.Module):
    ...
    def __init__(self, nf=64, in_c=16, norm="batch", max_frames=200, coarseness=10):
        ...
        self.encoder = nn.ModuleList([
            ...
            ConvBlock(nn.Conv2d, nf, nf * 2, ksize=4, stride=2, norm=norm, activation='leaky'),
            ConvBlock(nn.Conv2d, nf * 2, nf * 4, ksize=4, stride=2, norm=norm, activation='leaky'),
            ConvBlock(nn.Conv2d, nf * 4, nf * 4, ksize=4, stride=2, norm=norm, activation='leaky'),
            ConvBlock(nn.Conv2d, nf * 4, nf * 4, ksize=4, stride=2, norm=norm, activation='leaky'),
            ConvBlock(nn.Conv2d, nf * 4, nf * 4, ksize=4, stride=1, dil=2, norm=norm, activation='leaky'),
            ConvBlock(nn.Conv2d, nf * 4, nf * 4, ksize=4, stride=1, dil=2, norm=norm, activation='leaky')])
        self.decoder = nn.ModuleList([
            ConvBlock(nn.ConvTranspose2d, nf * 4 * 2, nf * 4, ksize=4, stride=2, norm=norm),
            ConvBlock(nn.ConvTranspose2d, nf * 4 * 2, nf * 4, ksize=4, stride=2, norm=norm),
            ConvBlock(nn.ConvTranspose2d, nf * 4 * 2, nf * 2, ksize=4, stride=2, norm=norm),
            ConvBlock(nn.ConvTranspose2d, nf * 2 * 2, nf, ksize=4, stride=2, norm=norm),
            ConvBlock(nn.ConvTranspose2d, nf * 2, nf, ksize=4, stride=2, norm=norm)])
