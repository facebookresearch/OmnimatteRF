...
import torch
from third_party.omnimatte.third_party.models.base_model import BaseModel
from third_party.omnimatte.third_party.models.networks_lnr import MaskLoss, cal_alpha_reg
from . import networks
import numpy as np
import torch.nn.functional as F
import third_party.omnimatte.utils as utils
...
class OmnimatteModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            # new options for experiments
            parser.add_argument('--supervise_n_samples', type=int, default=0, help='Only supervise this number of samples at every step')
            parser.add_argument('--supervise_n_fg_samples', type=int, default=0, help='If positive, use this number of samples only from the foreground region')

        # network normalization layers
        parser.add_argument('--net_norm', default="batch", choices=["none", "batch"], help='Which Norm2D layer to use in UNet')
    ...
    def __init__(self, opt):
        ...
        self.netOmnimatte = networks.define_omnimatte(opt.num_filters, opt.in_c, opt.net_norm, gpu_ids=self.gpu_ids)
    ...
    def setup_train(self, opt):
        ...
        # new options for experiments
        self.supervise_n_samples = opt.supervise_n_samples
        self.supervise_n_fg_samples = opt.supervise_n_fg_samples
    ...
    def backward(self):
        ...
        # experiment: supervise only at some samples
        n_samples = self.supervise_n_samples
        if n_samples <= 0:
            def sample(tensor):
                return tensor
            def loss(pred, target):
                return self.criterionLoss(pred, target)
        else:
            n_fg_samples = self.supervise_n_fg_samples
            H, W = self.reconstruction.shape[2:]

            if n_fg_samples <= 0:
                # get n_sample pixels for each image in batch
                indices = np.concatenate([np.random.choice(H*W, [n_samples], replace=False) for _ in range(b_sz)])
            else:
                # get n_fg_samples pixels in every foreground region,
                # and (n_samples - n_fg_samples) pixels in every bg region

                # get combined fg mask for each image
                fg_mask = self.mask[:, 1].cpu().clone() # [B, H, W]
                for i in range(2, self.mask.shape[1]):
                    fg_mask |= self.mask[:, i].cpu()

                # sample indices for every image in the batch
                indices = []
                for i in range(len(fg_mask)):
                    b_fg_mask = fg_mask[i].view(-1) > 0
                    b_indices_fg = torch.where(b_fg_mask)[0]
                    b_indices_bg = torch.where(~b_fg_mask)[0]
                    b_n_fg_samples = min(n_fg_samples, len(b_indices_fg))
                    indices += [
                        np.random.choice(b_indices_fg, [b_n_fg_samples], replace=False),
                        np.random.choice(b_indices_bg, [n_samples - b_n_fg_samples], replace=False),
                    ]

                indices = np.concatenate(indices)

            sample_b = torch.repeat_interleave(torch.arange(b_sz), (n_samples))
            sample_y = indices // W
            sample_x = indices % W

            def sample(tensor):
                if tensor.shape[0] == b_sz // 2:
                    # in this case the input is warped i+1 -> i, so
                    # use the samples of first half
                    n_half = sample_b.shape[0] // 2
                    return tensor[sample_b[:n_half], ..., sample_y[:n_half], sample_x[:n_half]]
                else:
                    return tensor[sample_b, ..., sample_y, sample_x]
            def loss(pred, target):
                return self.criterionLoss(sample(pred), sample(target))

        self.loss_recon = loss(self.reconstruction[:, :3], self.target_image)
        self.loss_alpha_reg = cal_alpha_reg(sample(self.alpha_composite * .5 + .5), self.lambda_alpha_l1, self.lambda_alpha_l0)

        alpha_layers = self.output_rgba[:, 3]
        self.loss_mask = self.lambda_mask * self.criterionLossMask(sample(alpha_layers), sample(self.mask))
        self.loss_recon_flow = self.lambda_recon_flow * loss(self.flow_confidence * self.reconstruction_flow, self.flow_confidence * self.flow_gt)

        alpha_t = alpha_layers[:b_sz // 2]
        self.loss_alpha_warp = self.lambda_alpha_warp * loss(self.alpha_warped[:, 0], alpha_t)

        rgb_t = self.target_image[:b_sz // 2]
        self.loss_recon_warp = self.lambda_recon_warp * loss(self.reconstruction_warped, rgb_t)

        brightness_reg = loss(self.brightness_scale, torch.ones_like(self.brightness_scale))
        offset_reg = sample(self.bg_offset).abs().mean()
