...
class MLPRender_Fea_TimeEmbedding(torch.nn.Module):
    ...
    def forward(self, pts, viewdirs, features, time=None):
        indata = [features]
        if self.feape > 0:
            indata += [positional_encoding(features, self.feape)]
...
class TensorBase(torch.nn.Module):
    ...
    def sample_ray_contracted(self, rays_o, rays_d, is_train=True, N_samples=-1):
        N_samples = N_samples if N_samples > 0 else self.nSamples
        near, far = self.near_far
        # t_vals = torch.linspace(0.0, 1.0, N_samples//2+1).unsqueeze(0).to(rays_o)
        # interpx_inner = t_vals * (2.0 - near) + near
        ...
        # @TODO coherent perturbation when training
        # if is_train:
        #     interpx += torch.rand_like(interpx).to(rays_o) * ((far - near) / N_samples)
    ...
    def forward(self, rays_chunk, white_bg=True, is_train=False, ray_contraction="none", N_samples=-1, return_everything_dict=False):
        assert ray_contraction in {"none", "ndc", "mipnerf"}

        # sample points
        viewdirs = rays_chunk[:, 3:6]
        if ray_contraction == "none":
            xyz_sampled, z_vals, ray_valid = self.sample_ray(rays_chunk[:, :3], viewdirs, is_train=is_train,N_samples=N_samples)
            dists = torch.cat((z_vals[:, 1:] - z_vals[:, :-1], torch.zeros_like(z_vals[:, :1])), dim=-1)
        elif ray_contraction == "ndc":
            xyz_sampled, z_vals, ray_valid = self.sample_ray_ndc(rays_chunk[:, :3], viewdirs, is_train=is_train,N_samples=N_samples)
            dists = torch.cat((z_vals[:, 1:] - z_vals[:, :-1], torch.zeros_like(z_vals[:, :1])), dim=-1)
            rays_norm = torch.norm(viewdirs, dim=-1, keepdim=True)
            dists = dists * rays_norm
            viewdirs = viewdirs / rays_norm
        elif ray_contraction == "mipnerf":
        ...
        match ray_contraction:
            case "none":
                depth_map = depth_map + (1. - acc_map) * rays_chunk[..., -1]
            case "ndc":
                depth_map = depth_map + (1. - acc_map) * (rays_chunk[..., 2] + rays_chunk[..., -1])
            case "mipnerf":
                depth_map = depth_map + (1. - acc_map) * 256

        if return_everything_dict:
            return {
                "rgb_map": rgb_map,
                "depth_map": depth_map,
