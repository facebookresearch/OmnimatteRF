...
def visualize_array(array, color_map=cv2.COLORMAP_INFERNO):
    """
...
def visualize_normalized_depths(depths: np.ndarray, color_map=cv2.COLORMAP_INFERNO):
    """Cololize normalized depth by computing the inverse 2/(d+1)-1
    depths: [H, W], values in range [0, 1]
    """
    # convert to inverse depths
    depths = 2 / (depths + 1) - 1
    return visualize_array(depths, color_map)
...
class TVLoss(nn.Module):
    ...
    def forward(self,x,ignore_axis=None):
        ...
        if ignore_axis is None:
            return self.TVLoss_weight*2*(h_tv/count_h+w_tv/count_w)/batch_size  # TODO: this line causing NaN due to count_h or count_w == 0
        elif ignore_axis == 'h':
