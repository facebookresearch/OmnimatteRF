...
class AlphaGridMask(torch.nn.Module):
    def __init__(self, device, aabb, alpha_volume, tSize):
        ...
        self.gridSize = torch.LongTensor([alpha_volume.shape[-2],alpha_volume.shape[-3],alpha_volume.shape[-4]]).to(self.device)
        self.tSize = tSize

    # def update_alpha(self, device, alpha_volume_t, t):
    #     self.device = device
    #     self.alpha_volume[..., t] = alpha_volume_t
    #     # self.alpha_volume = alpha_volume.view(1,1,*alpha_volume.shape[-3:])

    def sample_alpha(self, xyz_sampled, t):
        xyz_sampled = self.normalize_coord(xyz_sampled)
        # alpha_vals = F.grid_sample(self.alpha_volume, xyz_sampled.view(1,-1,1,1,3), align_corners=True).view(-1)
        alpha_vals = F.grid_sample(torch.permute(self.alpha_volume.to(xyz_sampled.get_device())[:, 0], (0, 4, 1, 2, 3)), xyz_sampled.view(1,-1,1,1,3), align_corners=True)
        t_int = torch.round((t+1)/2*(self.tSize-1)).long()
        t_int_onehot = torch.nn.functional.one_hot(t_int, num_classes=self.tSize)
        alpha_vals = torch.permute(alpha_vals[0, :, :, 0, 0], (1, 0))[t_int_onehot.bool()]
    ...
    def normalize_coord(self, xyz_sampled):
        return (xyz_sampled-self.aabb.to(xyz_sampled.get_device())[0]) * self.invgridSize.to(xyz_sampled.get_device()) - 1
...
class TensorBase(torch.nn.Module):
    def __init__(self, aabb, gridSize, tSize, device, density_n_comp = 8, appearance_n_comp = 24, app_dim = 27,
    ...
    def init_render_func(self, shadingMode, pos_pe, view_pe, fea_pe, featureC, device):
        if shadingMode == 'MLP_PE':
            ...
            self.renderModule = MLPRender_Fea_TimeEmbedding(self.app_dim, view_pe, fea_pe, featureC).to(device)
        elif shadingMode == 'MLP_Fea_late_view':
            self.renderModule = MLPRender_Fea_late_view(self.app_dim, view_pe, fea_pe, featureC).to(device)
        elif shadingMode == 'MLP_Fea_woView':
            self.renderModule = MLPRender_Fea_woView(self.app_dim, view_pe, fea_pe, featureC).to(device)
    ...
    def update_stepSize(self, gridSize, tSize):
        ...
        self.tSize = torch.unsqueeze(torch.tensor(tSize), 0).to(self.device)  # fix the voxel size in t dimension
    ...
    def normalize_coord(self, xyz_sampled):
        return (xyz_sampled-self.aabb.to(xyz_sampled.get_device())[0]) * self.invaabbSize.to(xyz_sampled.get_device()) - 1

    def unnormalize_coord(self, xyz_sampled):
        return (xyz_sampled + 1) / self.invaabbSize.to(xyz_sampled.get_device()) + self.aabb.to(xyz_sampled.get_device())[0]
    ...
    def load(self, ckpt):
        if 'alphaMask.aabb' in ckpt.keys():
            ...
            alpha_volume = torch.from_numpy(np.unpackbits(ckpt['alphaMask.mask'])[:length].reshape(ckpt['alphaMask.shape'])).to(self.device)
            self.alphaMask = AlphaGridMask(self.device, ckpt['alphaMask.aabb'].to(self.device), alpha_volume.float().to(self.device))
        self.load_state_dict(ckpt['state_dict'])
        # return ckpt['se3_poses'].to(self.device)
    ...
    def sample_ray_ndc(self, rays_o, rays_d, is_train=True, N_samples=-1):
        ...
        mask_outbbox = ((self.aabb.to(rays_pts.get_device())[0] > rays_pts) | (rays_pts > self.aabb.to(rays_pts.get_device())[1])).any(dim=-1)
    ...
    def sample_ray_contracted(self, rays_o, rays_d, is_train=True, N_samples=-1):
        ...
        # inner
        interpx_inner = torch.linspace(near, 2.0, inner_N_samples+1).unsqueeze(0).to(rays_o)
        if is_train:
            interpx_inner[:, :-1] += (torch.rand_like(interpx_inner).to(rays_o) * ((2.0 - near) / inner_N_samples))[:, :-1]
        ...
        interpx_outer = 1.0 / (1/(far) + (1/2.0 - 1/(far)) * rng / outer_N_samples).to(rays_o.device)
        interpx = torch.cat((interpx_inner, interpx_outer), -1)

        rays_pts = rays_o[..., None, :] + rays_d[..., None, :] * interpx[..., None]

        # rays_pts_norm = torch.norm(rays_pts.clone(), p=2, dim=-1)
    ...
    def getDenseAlpha(self,gridSize=None):
        ...
        alpha = torch.tile(torch.unsqueeze(alpha, -1), (1, 1, 1, int(self.tSize.item())))
        for i in range(gridSize[0]):
            for t_idx, t in enumerate(range(self.tSize.item())):
                alpha[i, :, :, t_idx] = self.compute_alpha(dense_xyz[i].view(-1,3), t / (self.tSize.item() - 1.0) * 2.0 - 1.0, self.stepSize).view((gridSize[1], gridSize[2]))
    ...
    def updateAlphaMask(self, gridSize=(200,200,200)):
        ...
        for t in range(self.tSize.item()):
            ...
            # self.alphaMask = AlphaGridMask(self.device, self.aabb, alpha[..., t])
        ...
        xyz_min, xyz_max = torch.stack(xyz_min_all_t, 0).amin(0), torch.stack(xyz_max_all_t, 0).amax(0)
    ...
    def forward(self, rays_chunk, ts_chunk, timeembeddings_chunk, xyz_sampled, z_vals, ray_valid, white_bg=True, is_train=False, ray_type="ndc", N_samples=-1):
        # sample points
        ...
        if ray_valid.any():
            ...
            if timeembeddings_chunk is not None:
                sigma_feature = self.compute_densityfeature(normalized_xyz_sampled[ray_valid], torch.tile(torch.unsqueeze(ts_chunk, -1), (1, normalized_xyz_sampled.shape[1]))[ray_valid], torch.tile(torch.unsqueeze(timeembeddings_chunk, 1), (1, normalized_xyz_sampled.shape[1], 1))[ray_valid])
            else:
                sigma_feature = self.compute_densityfeature(normalized_xyz_sampled[ray_valid], torch.tile(torch.unsqueeze(ts_chunk, -1), (1, normalized_xyz_sampled.shape[1]))[ray_valid], None)
        ...
        if app_mask.any():
            app_features = self.compute_appfeature(normalized_xyz_sampled[app_mask], torch.tile(torch.unsqueeze(ts_chunk, -1), (1, normalized_xyz_sampled.shape[1]))[app_mask], None)
            if timeembeddings_chunk is not None:
                valid_rgbs = self.renderModule(normalized_xyz_sampled[app_mask], viewdirs[app_mask], app_features, torch.tile(torch.unsqueeze(timeembeddings_chunk, 1), (1, normalized_xyz_sampled.shape[1], 1))[app_mask])
            else:
                valid_rgbs = self.renderModule(normalized_xyz_sampled[app_mask], viewdirs[app_mask], app_features, torch.tile(torch.unsqueeze(ts_chunk, -1), (1, normalized_xyz_sampled.shape[1]))[app_mask][..., None])
            rgb[app_mask] = valid_rgbs


        xyz_prime = self.warp_coordinate(xyz_sampled, torch.tile(torch.unsqueeze(ts_chunk, -1), (1, xyz_sampled.shape[1])))

        pts_ref = xyz_sampled

        if xyz_prime is None:
            return  None, None, None, pts_ref, weight, None, rgb, sigma, z_vals, dists * self.distance_scale


        if ray_valid.any():
            blending_features = self.compute_blendingfeature(normalized_xyz_sampled[ray_valid], torch.tile(torch.unsqueeze(ts_chunk, -1), (1, normalized_xyz_sampled.shape[1]))[ray_valid], None)
            blending[ray_valid] = torch.sigmoid(blending_features)[:, None]

        weights_d = weight
        return None, None, blending[..., 0], pts_ref, weights_d, xyz_prime, rgb, sigma, z_vals, dists * self.distance_scale
