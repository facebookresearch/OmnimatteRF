...
def get_ray_directions_lean(i, j, focal, center):
    ...
    # the direction here is without +0.5 pixel centering as calibration is not so accurate
    # see https://github.com/bmild/nerf/issues/24
    directions = torch.stack([(i - center[0]) / focal[0], -(j - center[1]) / focal[1], -torch.ones_like(i)],
                             -1)  # (b, 3)
...
def get_rays_lean(directions, c2w):
    ...
    rays_d = torch.bmm(directions[:, None, :], torch.transpose(c2w[:, :3, :3], 1, 2))[:, 0, :]  # (B, 3)
...
def get_rays_with_batch(directions, c2w):
    ...
    """
    # Rotate ray directions from camera coordinate to the world coordinate
    rays_d = torch.matmul(directions, torch.transpose(c2w[:, :3, :3], 1, 2))  # (B, H, W, 3)
    # rays_d = directions @ c2w[:3, :3].T  # (B, H, W, 3)
    # rays_d = rays_d / torch.norm(rays_d, dim=-1, keepdim=True)
    # The origin of all rays is the camera origin in world coordinate
    rays_o = torch.tile(torch.unsqueeze(c2w[:, :3, 3], 1), (1, directions.shape[1], 1))  # (B, H*W, 3)
