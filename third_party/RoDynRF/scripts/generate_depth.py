...
def run(input_path, output_path, output_img_path, model_path):
    ...
    for ind, img_name in enumerate(img_names):
        ...
        # output
        filename = str(ind).zfill(5)
        print(filename)

        np.save(os.path.join(output_path, filename + '.npy'), prediction.astype(np.float32))
        ...
        cv2.imwrite(os.path.join(output_img_path, filename + '.png'), out.astype("uint16"))
...
if __name__ == "__main__":
    ...
    input_path = os.path.join(args.dataset_path, 'rgb_1x')
    output_path = os.path.join(args.dataset_path, 'train_rodynrf/disp')
    output_img_path = os.path.join(args.dataset_path, 'train_rodynrf/disp_png')
